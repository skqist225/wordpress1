			...
			if( (in_array( $args->theme_location, $mega_menu_locations) ) && is_array( $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) && in_array( 'buddypress', $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) ) {
				if ( class_exists( 'BuddyPress' ) ){
					...
					if ( is_user_logged_in() ) {
						...
						if ( version_compare( $bp->version, '2.6' ) ) {
							// for BuddyPress versions that later than 2.6
							$bp = buddypress();
							$list_of_icons_for_bp = array(
								'activity' => 'health',
								'just-me' => 'health',
								'activity-mentions' => 'health',
								'activity-favs' => 'health',
								'xprofile' => 'user',
								'public' => 'user',
								'edit' => 'user',
								'change-avatar' => 'user',
								...
								'notifications-my-notifications' => 'notification-2',
								'read' => 'envelop-opened',
								'settings' => 'cog',
								'general' => 'cog',
								'profile' => 'cog',
							...
							foreach ( $bp->members->nav->get_item_nav() as $nav_menu ) {
								...
								// Add this menu.
								$buddypress_item .= mm_common::ntab(3) . '<li class="menu-item default_dropdown ' . strtolower( str_replace( ' ', '-', $nav_menu->css_id ) ) . ' ' . $drop_side . '">';
								$buddypress_item .= mm_common::ntab(4) . '<a href="' . $link . '" tabindex="0" class="item_link with_icon">';
								$buddypress_item .= mm_common::ntab(5) . '<i class="' . ( isset( $list_of_icons_for_bp[ $nav_menu->css_id ] ) ? 'im-icon-' . $list_of_icons_for_bp[ $nav_menu->css_id ] : 'im-icon-cog' ) . '"></i>';
								$buddypress_item .= mm_common::ntab(5) . '<span class="link_content">';
								$buddypress_item .= mm_common::ntab(6) . '<span class="link_text">' . $nav_menu->name . '</span>';
								...
								if ( ! empty( $nav_menu->children ) ) {
									...
									foreach( $nav_menu->children as $sub_menu ) {
										...
										if ( $sub_menu->slug == bp_current_action() ) {
											$submenu_class = ' current-menu-item';
										}
										$buddypress_item .= mm_common::ntab(5) . '<li class="menu-item default_dropdown ' . $drop_side . $submenu_class . '">';
										$buddypress_item .= mm_common::ntab(6) . '<a href="' . $sub_menu->link . '" tabindex="0" class="item_link">';
										$buddypress_item .= mm_common::ntab(7) . '<span class="link_content">';
										$buddypress_item .= mm_common::ntab(8) . '<span class="link_text">' . $sub_menu->name . '</span>';
							...
							// for BuddyPress versions that earlier than 2.6
							if ( version_compare( $bp->version, '2.5' ) ) {
								$bp_nav_items = (array) $bp->members->nav;
								$bp_nav_items = current($bp_nav_items);
								$bp_nav_items = current($bp_nav_items);
							} else {
								$bp_nav_items = $bp->bp_nav;
							}
							foreach ( $bp_nav_items as $key => $component ) {
								if ( isset( $component[ 'slug' ] ) ) {
									...
									$buddypress_item .= mm_common::ntab(3) . '<li class="menu-item default_dropdown ' . strtolower( str_replace( ' ', '-', $component[ 'name' ] ) ) . ' ' . $drop_side . '">';
